...
async function classifyPhotosServerSide(urls: string[], limit = 3) {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL ?? process.env.NEXTAUTH_URL ?? 'http://localhost:3000'}/api/vision/infer`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ photos: urls.slice(0, limit) })
    });
...
async function run() {
  ...
  for (const f of sampleFeatures) for (const k of Object.keys(f)) {
    const v = f[k];
  ...
  const photos = ((a as any).extractedListing?.photos as any) ?? [];
    ...
    const score = await classifyPhotosServerSide(photos, 3);
  ...
  const labeled = await prisma.scoreSnapshot.findMany({ where: { conditionScore: { not: null } }, take: 500 });
  for (const s of labeled) {
    const a = await prisma.analysis.findUnique({ where: { id: s.analysisId }, include: { featureSnapshot: true } });
    if (!a) continue;
  ...
  const artifact = { model: { weights, keys }, createdAt: new Date().toISOString(), samples: y.length };
  fs.writeFileSync(out, JSON.stringify(artifact, null, 2));

  // update latest
  const latestPath = path.join(modelDir, 'latest.json');
  let latest: any = { ts: new Date().toISOString(), vision: name };
  try { latest = JSON.parse(fs.readFileSync(latestPath, 'utf8')); latest.vision = name; latest.ts = new Date().toISOString(); } catch { /* ignore */ }
