...
try {
  cp.execSync('pnpm --dir d:/imob-ro/imob/imob-ro exec prisma migrate dev --name add-paywall-reportusage --create-only', { stdio: 'inherit' });
  cp.execSync('pnpm --dir d:/imob-ro/imob/imob-ro exec prisma generate', { stdio: 'inherit' });
  console.log('prisma migrate (create-only) + generate succeeded');
} catch (err) {
