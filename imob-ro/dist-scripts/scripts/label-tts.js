...
async function detectStatusForUrl(url) {
  try {
    ...
    if (status === 404 || status === 410) {
      return { state: "inaccessible", when: now, html: text };
    }
    // Some sites return 200 but show a "sold" label in HTML
    if (status === 200) {
      if (looksLikeSold(text)) return { state: "sold", when: now, html: text };
      return { state: "active", when: now, html: text };
    }
    // treat other non-2xx as inaccessible
    if (status < 200 || status >= 300) return { state: "inaccessible", when: now, html: text };
    return { state: "active", when: now, html: text };
  } catch (err) {
    return { state: "inaccessible", when: new Date(), html: "" };
  }
}
async function run() {
  ...
  for (const a of analyses) {
    try {
      const existing = await db_1.prisma.ttsLabel.findUnique({ where: { analysisId: a.id } });
      if (existing) {
        // skip existing
        continue;
      }
      ...
      await db_1.prisma.ttsLabel.create({ data: { analysisId: a.id, days, censored } });
