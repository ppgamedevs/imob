(function () {
  try {
    ...
    const fromJSONLD = (() => {
      ...
      const currency = offer.priceCurrency || offer?.priceSpecification?.priceCurrency || guessCurrency();
    ...
    const fromMeta = (() => {
      ...
      const price = parseNumber(desc && (desc.match(/(\d[\d\.\s,]{2,})\s?(?:EUR|€|RON|lei)/i) || [])[1]);
      const curMatch = desc && (desc.match(/\b(EUR|RON|lei|€)\b/i) || [])[1];
      const currency =
        curMatch ? (curMatch.toUpperCase() === "LEI" ? "RON" : curMatch === "€" ? "EUR" : curMatch.toUpperCase()) : guessCurrency();
    ...
    const fromSelectors = (() => {
      ...
      const currency = /RON|lei/i.test(priceText) ? "RON" : /EUR|€/.test(priceText) ? "EUR" : guessCurrency();

      const areaText = (document.body.innerText.match(/(\d{1,3}(?:[.,]\d{1,2})?)\s*(m2|m²|mp)/i) || [])[1];
      const areaM2 = parseNumber(areaText);
      const rooms = parseNumber((document.body.innerText.match(/\b(\d(?:\.\d)?)\s*(cam|camere|camera)\b/i) || [])[1]);

      const floorRaw = (document.body.innerText.match(/\b(Parter|Demisol|Mezanin|Etaj\s*\d{1,2}(?:\/\d{1,2})?)\b/i) || [])[0] || null;
      const yearBuilt = parseNumber((document.body.innerText.match(/\b(19\d{2}|20\d{2})\b/) || [])[1]);
      ...
      return { title: t, price, currency, areaM2, rooms, floorRaw, yearBuilt, addressRaw, lat, lng, photos };
    })();
