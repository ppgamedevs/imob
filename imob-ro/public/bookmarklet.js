(function () {
  ...
  function parseNumber(s){ if(!s) return null; var n=String(s).replace(/[^\d.,]/g,"").replace(/\.(?=.*\.)/g,"").replace(",",""); var f=parseFloat(n); return isFinite(f)?f:null; }
  var txt = document.body.innerText;
  var title = document.title;
  var m = txt.match(/(\d[\d\.\s,]{2,})\s?(EUR|€|RON|lei)/i);
  var price = m ? parseNumber(m[1]) : null;
  var currency = m ? (m[2].toUpperCase()==="LEI"?"RON":(m[2]==="€"?"EUR":m[2].toUpperCase())) : "EUR";
  var imgs = Array.from(document.querySelectorAll("img")).map(i=>i.currentSrc||i.src).filter(Boolean);
