...
export async function GET() {
  ...
  for (const a of analyses) {
    ...
    const target =
      a.featureSnapshot.features.priceEur ||
      a.group.analyses[0]?.scoreSnapshot?.avmMid * a.featureSnapshot.features.areaM2;
  ...
  const testMape =
    mean(testPreds.map((pred, i) => Math.abs((pred.prediction - test[i].y) / test[i].y))) * 100;
