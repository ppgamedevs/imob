...
export function estimateRent(features: any, compsRent: Array<number> | null) {
  ...
  const fallback = features?.estimated_rent_m2 ?? features?.rent_m2 ?? null;
...
export function computeYield(priceEur: number, rentPerMonthEur: number, capex: number = 0): YieldResult {
...
export default { estimateRent, computeYield };
