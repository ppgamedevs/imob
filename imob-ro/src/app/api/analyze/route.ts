...
import { checkListingUrl, getSupportedDomainsList } from "@/lib/analyze/check-listing-url";
...
const SUPPORTED = getSupportedDomainsList();
...
export async function POST(req: Request) {
  ...
  if (!allowRequest(`analyze:${ip}`)) {
    ...
    return NextResponse.json({ error: "rate_limited", reason: "rate_limited" }, { status: 429 });
  }
  ...
  if (!rawUrl) {
    return NextResponse.json(
      {
        error:
          "URL invalid sau nu securizat. Foloseste un link https complet, pana in 2048 de caractere, fara simboluri interzise.",
  ...
  if (!urlCheck.ok) {
    return NextResponse.json(
      { error: urlCheck.message, reason: urlCheck.reason, supportedDomains: SUPPORTED },
      { status: 400 },
    );
  }
